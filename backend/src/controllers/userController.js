...
export const createUser = async (req, res) => {
  try {
    ...
    if (!nombre || !correo || !password) return res.status(400).json({ msg: "Faltan campos obligatorios" });
    ...
    await Image.updateMany({ user: null, "meta.ip": ip, "meta.userAgent": userAgent }, { user: newUser._id });

    // Setear cookies con tokens
